...
# this loads the interests for those MPs of the current parliament. It will load this year's register. 
def LoadInterests():
        ...
        if 'Response' in response_json and response_json['Response'] is not None:
            for x in range(len(response_json['Response'])):
                try:
                    ...
                    if entry.get('AnsweringMinister') is not None:
                        answeringName = entry['AnsweringMinister'].get('Name', 'Unknown')
